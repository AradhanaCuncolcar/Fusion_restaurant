...
# Attempt to load the menu data as required by the AI Concierge
try:
    from menu_data import MENU
except ImportError:
    # Fallback if menu_data.py is not present
    MENU = []
...
# ---------------------------------------------------------
# Combined page config (using the wide layout from the dashboard and the icon from the AI tool)
...
# ---------------------------------------------------------
# 3. AI CONCIERGE FUNCTIONS & CONSTANTS
# ---------------------------------------------------------
...
def build_prompt(prefs: dict) -> str:
    return f"""You are the AI menu concierge for a Dubai fusion restaurant.

FULL MENU (JSON array, one object per dish):
{json.dumps(MENU, ensure_ascii=False)}

GUEST PREFERENCES (JSON):
{json.dumps(prefs, ensure_ascii=False)}

Pick exactly 3 dishes from the menu above that best match this guest. Rules:
- Never recommend a dish that conflicts with a listed dietary restriction.
- Respect their spice tolerance (don't push spice_level 2-3 dishes on a "Mild" guest).
- Weight toward their favorite cuisines and adventurousness score, but keep the
  set balanced (don't pick 3 desserts, mix categories sensibly for a real meal).
- Keep within a sensible reach of their budget preference where possible.
- "reason" should be 1 short sentence, written warmly and specifically, in the
  voice of a knowledgeable host talking directly to the guest.
- "pairing" is an optional 1-sentence suggestion, or null if nothing fits.

Respond with ONLY valid JSON, no markdown fences, no commentary:
{{
  "recommendations": [
    {{"dish_id": "...", "name": "...", "price_aed": 0, "reason": "...", "pairing": "..." }}
  ],
  "concierge_note": "1-2 warm sentences introducing the picks as a set"
}}
"""

def call_claude(prefs: dict) -> dict:
    api_key = get_api_key()
    if not api_key:
        raise RuntimeError("No ANTHROPIC_API_KEY found in secrets or environment")
    ...
    message = client.messages.create(
        ...
        max_tokens=800,
        temperature=0.4,
        messages=[{"role": "user", "content": build_prompt(prefs)}],
    )
...
# =========================================================
# TAB 7: AI Menu Concierge
# =========================================================
with tab_ai:
    # Use columns to narrow the layout inside the wide dashboard
    ...
    with col2:
        ...
        st.caption("Answer a few quick questions and Claude will pick 3 dishes for you from tonight's menu.")
